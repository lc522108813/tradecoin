package com.leichuang.tradecoin.consts;

import java.math.BigDecimal;

public class CoinConsts {

    public static final Integer SUCCESS_CODE = 200;

    public static final String BINANCE = "binance";

    public static final String HUOBI = "huobi";

    public static final String BINANCE_INTERVAL_ONE_MINUETE = "1m";

    public static final String BINANCE_INTERVAL_THREE_MINUETE = "3m";

    public static final String BINANCE_INTERVAL_FIVE_MINUETE = "5m";

    public static final String BINANCE_INTERVAL_FIFTEEN_MINUTE = "15m";

    public static final String BINANCE_INTERVAL_THIRTY_MINUTE = "30m";

    public static final String BINANCE_INTERVAL_ONE_DAY = "1d";

    public static final String BINANCE_INTERVAL_ONE_HOUR = "1h";

    public static final String BINANCE_NTERVAL_ONE_MONTH = "1M";

    public static final String HUOBI_PERIOD_ONE_DAY = "1day";
    public static final String HUOBI_PERIOD_ONE_MINUTE = "1min";
    public static final String HUOBI_PERIOD_HALF_HOUR = "30min";

    public static final Integer LIMIT_1 = 1;
    public static final String USDT_SYMBOL = "usdt";


    public static final BigDecimal BIG_DEC_100 = new BigDecimal(100);


}
